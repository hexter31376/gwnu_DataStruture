...
Datatype* infix_to_postfix(Stack* stack, Datatype* input){
    char inputValue, operater; // input의 inputIndex번 인덱스의 값을 저장할 변수 inputValue, 

    int inputLength = strlen(input); // input의 길이
    Datatype* output = (Datatype*)malloc(sizeof(Datatype) * (inputLength + 1)); // output 동적 할당

    int outputIndex = 0; // 오직 output에 집어넣었을때만 증가
    int inputIndex = 0; // 오직 input을 스캔할때마다 증가

    while(inputIndex < inputLength){
        inputValue = input[inputIndex++]; // input의 inputIndex인덱스의 값을 invalue에 저장하고 카운터 1 증가

        if('A' <= inputValue && inputValue <= 'Z'){ // invalue가 알파벳 대문자 범위이면 // 그냥 숫자이면
            output[outputIndex++] = inputValue; // invalue를 outputIndex에 집어넣고 카운터 1 증가
        } else if(inputValue == ')'){ // invalue가 )이면 // 오른쪽 괄호
            while((operater = pop_stack(stack)) != '('){ // pop을 하고 그 값을 operater에 집어넣은 다음 그값이 ( 가 아니면
                output[outputIndex++] = operater; // 스택에서 나온 연산자를 저장한 operater 변수를 output에 집어넣고 카운터 1 증가
            }
            ...
            while (!(stack->count == 0) && in_prec(seeTop_stack(stack)) >= out_prec(inputValue)){ // 스택이 비지 않았고 안쪽 오퍼레이터의 우선순위가 바깥쪽 오퍼레이터의 순서보다 더 높다면
                output[outputIndex++] = pop_stack(stack); // 스택을 pop 하고 그 값을 output의 outputIndex에 저장하고 카운터 1 증가
            }
    ...
    // 스택에 남아있는 연산자 처리
    while (!(stack->count == 0)){
        output[outputIndex++] = pop_stack(stack);
    }
 ...
 * 기본 규칙
 * 1. 숫자 스캔시 그냥 출력
 * 2. 연산자 스캔시 다음과 같은 절차를 진행한다
 * 3. 스택 안쪽 연산자와 비교하여 스택 안쪽 연산자보다 바깥쪽 연산자가 크다면 스택에 집어넣는다.
 * 4. 그 반대이면 그대로 출력한다
 * 세부 규칙
 * 1. (를 만나면 무조건 집어넣는다
 * 2. )를 만나면 (를 만날때까지 모든 데이터를 pop하여 그 데이터를 나온 순서대로 출력한다
 */